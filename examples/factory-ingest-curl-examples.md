...
Use these one at a time after either local runtime path is running.
...
`POST /factory/ingest` starts a Temporal workflow and returns a `workflow_id`.
...
These do not call NVIDIA because the factory rejects unsupported input before
model validation.
